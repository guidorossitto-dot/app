...
(() => {
  ...
  function categoryLabel(cat) {
    switch (cat) {
      ...
      case "theatre": return "🎭 Teatro";
      case "visual_arts": return "🖼️ Visuales";
      default: return "";
    }
  }

  function categoryTagHTML(ev) {
    const t = categoryLabel(ev?.category);
  ...
  // Evita que tus handlers de drag/zoom cierren popups cuando el movimiento viene de UI
...
function glowMarker(marker) {
  ...
  /* =========================
     REBUILD LOCATION MARKERS (HOY + cluster) + popup ficha
  ========================= */
  ...
  Compartir
      ...
      // ✅ Delegación robusta + highlight al abrir popup
      loc.marker.on("popupopen", (evt) => {
        ...
        // Delegación: 1 solo listener para todos los botones del popup
        const onClick = async (ev) => {
          ...
          // 1) CENTRAR: solo cámara (cluster-safe) — no toca filtros
          ...
          // 2) COMPARTIR
          // 2) COMPARTIR (pro: incluye título)
// 2) COMPARTIR
if (btn.classList.contains("popupShareBtn")) {
  ...
  return;
        ...
        // ✅ Highlight al abrir (no al clickear)
  ...
  /* =========================
     RADIO FILTER (HOY + categoría + distancia)
  ========================= */
  function filterEventsByDistance(lat, lng) {
    const active = state.activeCategory;

    const base0 = getTodayEvents(state.events);
    const base = (active && active !== "all")
      ? base0.filter(ev => ev.category === active)
      : base0;

    const filtered = base.filter(ev => {
      if (!util.isValidCoord(ev.lat) || !util.isValidCoord(ev.lng)) return false;
      return util.distanceKm(lat, lng, ev.lat, ev.lng) <= App.CFG.SEARCH_RADIUS_KM;
    });

    App.ui.updateNearbyCount(filtered);
    App.ui.renderEvents();
    App.ui.renderNearbyEvents(filtered);
  ...
  /* =========================
     USER LOCATION + EVENT CREATION MARKER
  ========================= */
  ...
  function createEventFromAdminForm() {
    ...
    const category = util.normalizeCategory(catEl ? catEl.value : "music");

    if (!title || !date || !util.isValidCoord(lat) || !util.isValidCoord(lng)) {
      alert("Completá título, fecha y coordenadas válidas.");
      return;
    }

    state.events.push({ id: util.newId(), title, date, lat, lng, placeName, startTime, category });

    titleEl.value = "";
    if (startEl) startEl.value = "";

    storage.saveEvents();
    App.ui.commit({ rebuildMarkers: true, keepDistanceFilter: true });
  }
  ...
  // reconstruir markers HOY (tu lógica actual)
  ...
  // Mover cámara (sin tocar filtros)
  uiSetView(lat, lng, targetZoom);

  // Abrir popup cluster-safe si existe marker
  if (loc?.marker) {
    if (state.markerCluster && typeof state.markerCluster.zoomToShowLayer === "function") {
      state.markerCluster.zoomToShowLayer(loc.marker, () => {
  ...
  /* =========================
     DEEP LINK TARGET: foco por eventId
  ========================= */
 function focusEventById(eventId) {
  ...
  // Si la categoría actual oculta el evento, no lo escondas por filtro en un deep-link
  // (no tocamos chips acá; eso lo hace ui-calendar)
  // Limpieza de marker temporal previo
  ...
  // ✅ Caso ideal: el marker "normal" existe (ej: evento de hoy ya marcado)
  if (loc?.marker) {
  ...
  // ✅ Fallback robusto: evento NO está en los markers de HOY → marker temporal
  // Esto permite deep-link a eventos futuros/pasados (si existen) sin romper tu lógica de "HOY".
  ...
  // Usamos tu icono por categoría si existe (getCategoryIcon está en tu archivo)
  // Si no existe por algún cambio, Leaflet usa el default.
  let markerOpts = {};
  try {
  ...
  m.bindPopup(html, { closeButton: true, autoPan: true, keepInView: true, autoPanPadding: [30, 30], offset: [0, -10] });
  ...
  // Efectito opcional: glow (si querés)
  setTimeout(() => {
