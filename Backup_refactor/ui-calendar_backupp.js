...
(() => {
  ...
  function filterByCategory(list) {
    ...
    return (list || []).filter((ev) => ev?.category === c);
  }
  ...
  function renderEvents(list = state.events) {
    ...
    const today = util.todayStrYYYYMMDD();
    const onlyFuture = filterByCategory((list || []).filter((ev) => ev?.date && ev.date > today));
  ...
  function renderTodayEvents() {
    ...
    const todayStr = util.todayStrYYYYMMDD();
    const todayEvents = filterByCategory((state.events || []).filter((ev) => ev?.date === todayStr));
  ...
  function renderEventsIntoUl(ulId, list, emptyMsg) {
    ...
    const sorted = [...list].sort(util.sortEventsByStatusThenTime);
  ...
  /* =========================
     NEARBY HEADER + FEATURED + MASTER ACCORDION
  ========================= */
  function updateNearbyCount(list) {
    ...
    const catChip = cat === "all" ? "" : `<span class="miniChip">${categoryLabel(cat)}</span>`;
    ...
    function featuredRank(ev) {
      ...
      // en curso (hasta 3h hacia atrás)
      if (m <= 0 && m > -180) return 0 + Math.abs(m) / 1000;
    ...
    // ===== master accordion (siempre cerrado) =====
    el.innerHTML = `
      ${featuredHTML}
  ...
  function renderAll(opts = {}) {
    ...
    if (rebuildMarkers && App.map?.rebuildLocationMarkers) App.map.rebuildLocationMarkers();

    if (keepDistanceFilter) {
      const pos = state.userMarker ? state.userMarker.getLatLng() : null;
      if (pos && App.map?.filterEventsByDistance) {
        App.map.filterEventsByDistance(pos.lat, pos.lng);
      } else {
  ...
  function bindLoginUI() {
    ...
    if (loginBtn) {
      loginBtn.addEventListener("click", () => {
        ...
        renderApp();
        App.ui.renderAll({ rebuildMarkers: true, keepDistanceFilter: true });
      });
    }
  ...
  /* =========================
     DEEP LINK (#e=EVENT_ID) — queue + process
  ========================= */
  function queueDeepLinkFromHash() {
  const h = (location.hash || "").replace(/^#/, "");
  if (!h) return;

  // acepta: e=ID y opcional t=Titulo
...
function processQueuedDeepLink() {
  ...
  // ✅ Deep-link no debe quedar oculto por el filtro de categoría
  if (
